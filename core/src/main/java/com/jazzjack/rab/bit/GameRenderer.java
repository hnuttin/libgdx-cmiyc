...
public class GameRenderer extends OrthogonalTiledMapRenderer {
    ...
    GameRenderer(GameObjectProvider gameObjectProvider, GameAssetManager assetManager, float scale) {
        ...
        lightBuffer = new FrameBuffer(Pixmap.Format.RGBA8888, Gdx.graphics.getWidth(), Gdx.graphics.getHeight() - 64, false);
    }
    ...
    private void renderSight() {
        gameDrawer.drawWithBlendFunction(GL20.GL_ZERO, GL20.GL_SRC_COLOR, () -> batch.draw(lightBuffer.getColorBufferTexture(), 0, 64));
    }
        ...
        if (optionalPlayer.isPresent()) {
            ...
            for (int i = 0; i < optionalPlayer.get().getHp(); i++) {
                gameDrawer.drawInStatusBarRegion(
                        assetManager.getHitpointTexture(),
                        i * getScaledTileWidth(),
                        0,
                        getScaledTileWidth(),
                        getScaledTileHeight());
            }
